defmodule RelativeDistance do
  ...
  def degree_of_separation(family_tree, person_a, person_b) do
   ...
   firstway = getparent(keys, family_tree, person_a, keys)
   twoway = getparent(keys, family_tree, person_b, keys)
   # other way
   IO.inspect(firstway)
   IO.inspect(twoway)
 ...
 defp getparent([], family_tree, person_a, keys), do: []
 defp getparent([head|tail], family_tree, person_a, keys) do
    if  person_a in family_tree[head] do
       [head] ++ getparent(keys, family_tree, head, keys)
    else
       getparent(tail, family_tree, person_a, keys)
    end
 end
